...
namespace Mitsuba2SeeSharp {
    public static class SpectralMapper {
        internal const float CIEWavelengthStart = 390;
        internal const float CIEWavelengthEnd = 830;
        internal const float CIEYNorm = 113.042314572337f; // Integral over CIE_Y
        internal const float D65WavelengthStart = 300;
        internal const float D65WavelengthEnd = 830;

        /// <summary>
        /// Maps CIE XYZ to sRGB with D65 as white point
        /// </summary>
        public static (float, float, float) MapRGB(float x, float y, float z) {
            float r = 3.2404542f * x - 1.5371385f * y - 0.4985314f * z;
            float g = -0.9692660f * x + 1.8760108f * y + 0.0415560f * z;
            float b = 0.0556434f * x - 0.2040259f * y + 1.0572252f * z;

            return (MathF.Max(0, r), MathF.Max(0, g), MathF.Max(0, b));
        }

        /// <summary>
        /// Returns CIE XYZ color triplet for a given wavelength
        /// </summary>
        public static (float, float, float) Eval(float[] wvls, float[] weights, bool emissive) {
            float x = 0, y = 0, z = 0;
            for (int i = 0; i < wvls.Length - 1; ++i) {
                float aW = wvls[i];
        ...
        internal static float LookupCIE(float wvl, float[] array) {
            return Lookup(wvl, array, CIEWavelengthStart, CIEWavelengthEnd);
        }

        internal static float LookupD65(float wvl) {
            return Lookup(wvl, CIE_D65, D65WavelengthStart, D65WavelengthEnd);
        }

        internal static float Lookup(float wvl, float[] array, float start, float end) {
            if (wvl < start || wvl > end) return 0;

            float delta = (end - start) / (array.Length - 1);
            float af = MathF.Max(0.0f, (wvl - start) / delta);
            int index = (int)MathF.Min((float)array.Length - 2, af);
            float t = MathF.Min((float)array.Length - 1, af) - index;

            return array[index] * (1 - t) + array[index + 1] * t;
        }

        internal static float[] CIE_X = {
        ...
        internal static float ComputeNormFactorD65() {
            List<float> wavelengths = new();

            for (int i = 0; i < CIE_Y.Length; ++i) {
                float wvl = (CIEWavelengthEnd - CIEWavelengthStart) * i / (CIE_Y.Length - 1) + CIEWavelengthStart;
                wavelengths.Add(wvl);
            }

            for (int i = 0; i < CIE_D65.Length; ++i) {
                float wvl = (D65WavelengthEnd - D65WavelengthStart) * i / (CIE_D65.Length - 1) + D65WavelengthStart;
                if (wvl >= CIEWavelengthStart) wavelengths.Add(wvl);
            }

            float[] uniqueWvls = wavelengths.ToHashSet().ToArray();
            float trapz = 0;
            for (int i = 0; i < uniqueWvls.Length - 1; ++i) {
                float aY = Lookup(uniqueWvls[i], CIE_Y, CIEWavelengthStart, CIEWavelengthEnd);
                float aI = LookupD65(uniqueWvls[i]);
                float bY = Lookup(uniqueWvls[i + 1], CIE_Y, CIEWavelengthStart, CIEWavelengthEnd);
                float bI = LookupD65(uniqueWvls[i + 1]);

                float a = aI * aY;
                float b = bI * bY;
                float dx = uniqueWvls[i + 1] - uniqueWvls[i];
                trapz += dx * (b + a) / 2;
            }
        ...
        internal static float CIEYNormD65 = ComputeNormFactorD65();
    }
}
